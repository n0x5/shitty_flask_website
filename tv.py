...
def wiki_article(search=None, search2=None):
    ...
    final_string7 = final_string6.replace('images/', 'https://img.domain111.org/{}_images/' .format(search2)).replace('{{cite}}', '').replace('{{Cite}}', '')
...
def wiki_search(search=None, search2=None):
    ...
    count = len(results)
    conn.close()
    return render_template('wiki_search.html', results=results, count=count, search=search, search2=search2)

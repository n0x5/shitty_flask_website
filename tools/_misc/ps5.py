...
i2 = r'List of PlayStation 5 games - Wikipedia.html'
...
for item in list1:
    title2 = item.find('th', attrs={'scope': 'row'})
    title = title2.get_text().strip()
    tds = item.find_all('td')
    genre = tds[0].get_text().strip()
    developer = tds[1].get_text().strip()
    publisher = tds[2].get_text().strip()
    eu = tds[5].get_text().strip()
    jp = tds[3].get_text().strip()

...
i2 = r'List of PlayStation 4 games (M–Z) - Wikipedia.html'
...
for item in list1:
    ...
    sql = 'select title, systems from games where title = ? order by dated desc'
    ...
    eu = tds[6].get_text().strip()
    jp = tds[4].get_text().strip()
    na = tds[5].get_text().strip()
    orig_system = 'ps4'
    yearscomb = eu, jp, na
    try:
